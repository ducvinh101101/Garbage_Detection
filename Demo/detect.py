...
# Agent pipeline đã được viết trong agent_core.py
...
# CLASS_NAMES = {0: "battery", 1: "biological", 2: "cardboard", 3: "clothes", 4: "glass", 5: "metal", 6: "paper", 7: "plastic", 8: "shoes", 9: "trash"}
    ...
    # Thêm biến trạng thái cho flow phân loại nhựa
    mode = "NORMAL"
    ...
    while True:
            ...
            for box, track_id in zip(boxes, track_ids):
                ...
                if track_id in final_predictions:
                    ...
                    if predicted_class == "plastic":
                        ...
                        if mode == "NORMAL":
                            display_text += " -> Nhan 'y' de phan loai"
                        elif mode == "CAPTURE":
        ...
        # Hiển thị thông báo khi ở chế độ chụp
        if mode == "CAPTURE":
            cv2.putText(frame, "CHE DO CHUP: Can chinh va nhan 'c' de chup, 'esc' de huy", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
        ...
        if mode == "NORMAL":
            if key == ord('y') and plastic_detected_crop is not None:
                mode = "CAPTURE"
        elif mode == "CAPTURE":
            if key == 27: # Phím ESC
                mode = "NORMAL"
                ...
                # Kích hoạt Agent xử lý ảnh này bằng LangChain Agent Pipeline
                run_agent_pipeline(plastic_detected_crop)
                mode = "NORMAL" # Reset lại trạng thái sau khi xong
